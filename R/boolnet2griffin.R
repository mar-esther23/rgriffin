...
get.net.topology <- function(net) {
  ...
  for (i in net$genes) {
    for (j in net$genes) {
      sign = get.interaction.sign(j, i, net)
      if (!is.null(sign)) {
...
#' @keywords internal
#' 
get.interaction.sign <-  function(source, target, net) {
...
#' @export
attractor2dataframe <- function(attr, sep="/", node.names=NULL, Boolean=TRUE) {
  if(  !("BoolNet" %in% (.packages()))  ) warning("BoolNet is not attached")
  if (Boolean) {
